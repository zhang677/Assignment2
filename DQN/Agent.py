...
from model import DeepQNetwork, LinearQNetwork
...
class DQNAgent:
    ...
    def choose_action_cuda(self, observation):
        self.eval_network.eval()
        with torch.no_grad():
            actions = self.eval_network.forward(observation)
    ...
    def double_learn(self):
        ...
        next_actions = self.choose_action_cuda(next_states)
        self.target_network.eval()
        with torch.no_grad():
